...
class DirectorService {
	 ...
	 * @return director
	 * @throws EntityNotFoundException when the director does not exist
	 */
	public Director getById(Long id) {
		return directorRepository.findById(id)
				.orElseThrow(() -> new EntityNotFoundException("Director not found: " + id));
	}
	...
	public Director update(Long id, Director director) {
		if (!directorRepository.existsById(id)) {
			throw new EntityNotFoundException("Director not found: " + id);
		}
		director.setId(id);
		return directorRepository.save(director);
	}
